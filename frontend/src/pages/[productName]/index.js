...
export default function ProductDetails() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await axios.get(`https://mern-e-commerce-production-10a6.up.railway.app/product/${slug}`);
        setProduct(res.data);
  ...
  const handleReviewSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('https://mern-e-commerce-production-10a6.up.railway.app/api/reviews', {
        product: product._id,
        ...reviewForm
      });
        ...
        <Col md={6}>
          <Row xs={2} md={3} className="g-3">
            {product.image.map((img, index) => {
              const imgUrl = img.startsWith('http') ? img : `https://mern-e-commerce-production-10a6.up.railway.app${img}`;
              return (
                <Col key={index}>
                  <Card
          ...
          <Card className="mt-4 border-0 shadow-sm">
            <Card.Img
              src={product.image[selectedImage].startsWith('http') ? product.image[selectedImage] : `https://mern-e-commerce-production-10a6.up.railway.app${product.image[selectedImage]}`}
