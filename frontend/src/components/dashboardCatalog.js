...
export default function AdminProductsDashboard() {
    ...
    const fetchData = async () => {
      try {
        const [productsRes, categoriesRes] = await Promise.all([
          axios.get('https://mern-e-commerce-f9ra.onrender.com/catalog', {
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
            }
          }),
          axios.get('https://mern-e-commerce-f9ra.onrender.com/categories', {
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
            }
          })
    ...
    try {
      if (category === 'all') {
        ...
        const res = await axios.get(
          `https://mern-e-commerce-f9ra.onrender.com/category/${category}`,
          {
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
            }
  ...
  const getProductImage = (product) => {
    ...
    return `https://mern-e-commerce-f9ra.onrender.com${product.image[0]}`;
  };
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(
        `https://mern-e-commerce-f9ra.onrender.com/delete/${selectedProduct._id}`,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
          }
