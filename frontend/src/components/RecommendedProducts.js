...
const RecommendedProducts = ({ currentProductId, category }) => {
  ...
  useEffect(() => {
    const fetchRecommendedProducts = async () => {
      try {
        const res = await axios.get(`https://sublime-magic-production.up.railway.app/category/${encodeURIComponent(category)}`);
        ...
        {products.map((product) => (
          <Col key={product._id} xs={6} md={3}>
            <Card as={Link} to={`/product/${product.slug}`} className="h-100 text-decoration-none">
              <Card.Img
                variant="top"
                src={`https://sublime-magic-production.up.railway.app${product.image[0]}`}
                alt={product.name}
                style={{
                  height: '180px',
                  objectFit: 'cover'
